...
"px";a.style.left="-100px";a.style.height=Math.floor(b.height)+"px";a.style.visibility="visible";$(a).animate({left:Math.ceil(b.right)+"px"},{complete:function(){app.map._map.updateSize()}})}},!1);d=document.createElement("div");d.className="search ol-custom ol-unselectable ol-control";d.appendChild(c);ol.control.Control.call(this,{element:d,target:a.target})};ol.inherits(searchControl,ol.control.Control);
...
function getForecast(a,b,c){c||(c=app.tzOffset);navigator.userAgent.match(/mobile/i)&&app.hide_clock();var d=document.getElementById("map"),e=Math.min(window.innerWidth,window.innerHeight)/20,k=(new Spinner({length:e,radius:e})).spin(d);forecast(a,b,function(d,e){k.stop();e=d?d.message:formatForecast(e,a,b,c);alertify.alert(e);if(!d&&!isFullScreen()&&(d=document.getElementById("alertify-ok"))){e="http://marine.weather.gov/MapClick.php?lat="+b+"&lon="+a;var f=document.createElement("a");f.target="_blank";
f.href=e;f.innerHTML="Marine Weather";f.className="alertify-button alertify-button-ok";d.parentElement.appendChild(f)}if(d=document.getElementById("forecast"))d.style.height=.6*window.innerHeight+"px"})}
...
(app.clock.positioned=0,positionClock()),app.map.updateFeatures(!0))}},onNeedsCalibration:function(){alertify.alert("Compass needs calibration")},timeout:locationTimeout});
window.onload=function(){initialize();updateTimeout=setTimeout(updateDefault,locationTimeout);startClock();swipedetect(clock.canvas,function(a){geolocation.changeSpeedUnit()},!0);geolocation.start();window.addEventListener("click",app.hideSearch,!0);window.addEventListener("touchstart",app.hideSearch,!0);window.addEventListener("orientationchange",handleDeviceOrientation)};
